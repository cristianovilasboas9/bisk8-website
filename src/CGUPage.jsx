...
.legal-content .company-data .bisk8-inline { height: 16px; opacity: 0.7; margin-left: 0; display: inline-block; vertical-align: baseline; }
...
function processBisk8Logos(html) {
  ...
  // Replace BISK8 in company data paragraphs (containing address info)
  result = result.replace(/<p>([\s\S]*?)<\/p>/g, (match, content) => {
    if (content.includes('Barcelos') || content.includes('contact@bisk8.co')) {
      return '<p class="company-data">' + content.replace(/BISK8/g, logo) + '</p>';
    }
    return match;
  });
  return result;
      ...
      <footer style={{ padding: "48px 32px", borderTop: "1px solid #111" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
          ...
          <div style={{ display: "flex", gap: 24 }}>
            <a href="/terms" style={{ color: "#666", textDecoration: "none", fontSize: 13 }}>Terms</a>
